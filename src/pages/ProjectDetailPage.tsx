...
const statusLabels: Record<string, string> = {
  planning: 'Планирование', in_progress: 'В работе', on_hold: 'На паузе', completed: 'Завершён',
};

const stepStatusIcons: Record<string, React.ElementType> = {
  pending: Circle, in_progress: Play, completed: CheckCircle2, skipped: Clock,
};
const stepStatusColors: Record<string, string> = {
  pending: 'text-muted-foreground', in_progress: 'text-primary', completed: 'text-green-600', skipped: 'text-muted-foreground/50',
};
...
export default function ProjectDetailPage() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-4xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="font-medium mt-1">{owner?.full_name || '—'}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Этапы проекта</CardTitle>
        </CardHeader>
        <CardContent>
