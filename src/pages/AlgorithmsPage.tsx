...
const iconMap: Record<string, React.ElementType> = {
  Cog, Monitor, Users, Factory, Network,
};
...
export default function AlgorithmsPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Библиотека алгоритмов</h1>
        <p className="text-muted-foreground">Выберите шаблон трансформации для создания проекта</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {mockTemplates.map(template => {
          const steps = mockTemplateSteps.filter(s => s.template_id === template.id);
            ...
            <Card key={template.id} className="group hover:shadow-lg transition-shadow border-border/50">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <p className="text-xs font-medium text-muted-foreground">Этапы ({steps.length}):</p>
                  {steps.map(step => (
                ...
                <Button className="w-full" onClick={() => navigate('/projects/new?template=' + template.id)}>
                  Создать проект <ArrowRight className="h-4 w-4 ml-1" />
                </Button>
              </CardContent>
            </Card>
          );
